...
const MAX_TWO_TRANSFER_OPTIONS = 300; // safety cap to prevent heap OOM with 1.3M transfers
...
/**
 * Computes multi-leg transit journeys between an origin and a destination.
 *
 * Pipeline: candidate lines → direct routes → 1-transfer routes →
 * 2-transfer routes → rank and dedupe → top results.
 *
 * Pure logic over a precomputed {@link TransferCacheService} snapshot —
 * no per-request database access. The cache must be loaded (auto-loaded
 * at module bootstrap) before `search()` is called, otherwise results
 * are silently empty.
 */
@Injectable()
export class SearchService {
  constructor(
    /**
     * Precomputed line + transfer network. Must be initialized before
     * `search()` is called (auto-loaded at module bootstrap).
     */
  ...
  /**
   * @param dto  Origin/destination as `"lat,lng"` strings, walk radius,
   *             max transfers, result limit, and whether to include
   *             polylines in the response.
   * @returns    Up to `dto.limit` journeys sorted by estimated time.
   *             Empty array (not throw) when no line is reachable from
   *             either endpoint. Non-direct options must beat the best
   *             direct option by at least the improvement threshold
   *             (see {@link IMPROVEMENT_THRESHOLD_PERCENT} /
   *             {@link IMPROVEMENT_THRESHOLD_MIN}).
   */
  async search(dto: SearchRouteDto): Promise<JourneyOption[]> {
    ...
    const originRadius = dto.originRadius ?? MAX_WALK_ORIGIN;

    console.log('=== SEARCH DEBUG ===');
    console.log('Origin:', origin);
    console.log('Destination:', destination);
    console.log('Origin radius:', originRadius);

    // 1. Filtrado geográfico
    const originCandidates = this.findCandidateLines(
      origin,
      destination,
      originRadius,
    );
    const destCandidates = this.findCandidateLines(
      destination,
      origin,
  ...
  private findCandidateLines(
    near: [number, number],
    target: [number, number],
    radiusMeters: number,
  ): CandidateLine[] {
    const lines = this.transferCache.getAllLines();
    const candidates: CandidateLine[] = [];

    for (const line of lines) {
      const boardResult = this.findClosestPoint(line.polyline, near);
      const alightResult = this.findClosestPoint(line.polyline, target);
  ...
  private async buildTwoTransferRoutes(
    ...
    for (const line1 of originCandidates) {
      ...
      for (const transfer1 of transfers1) {
        ...
        const line2Data = this.transferCache.getLineData(transfer1.lineBId) ?? null;
        ...
        for (const transfer2 of transfers2) {
          ...
          // Walk-time pruning: if the total walk time alone already exceeds the
          // best option's total time seen so far, this 2-transfer route can't
          // beat it — skip the (expensive) polyline slicing.
          const originWalk = line1.boardDistance > 0 ? Math.round(line1.boardDistance) : 0;
          const destWalk = line3Dest.boardDistance > 0 ? Math.round(line3Dest.boardDistance) : 0;
          const totalWalk = originWalk + Math.round(transfer1.walkDistance) + Math.round(transfer2.walkDistance) + destWalk;
          const estimatedWalkTime = totalWalk / WALK_SPEED;

          if (minEstimatedTime !== Infinity && estimatedWalkTime > minEstimatedTime) continue;
          ...
          const ride1Dist = Math.round(this.calculatePolylineDistance(ride1Polyline));
          const ride2Dist = Math.round(this.calculatePolylineDistance(ride2Polyline));
          const ride3Dist = Math.round(this.calculatePolylineDistance(ride3Polyline));
          ...
          const option = this.buildJourneyOption(legs, 2);
          options.push(option);
          if (option.estimatedTime < minEstimatedTime) {
            minEstimatedTime = option.estimatedTime;
          }
        }
      }
    }
